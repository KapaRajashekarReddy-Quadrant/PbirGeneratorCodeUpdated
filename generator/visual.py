...
class VisualGenerator:
    ...
    # Maps a generic internal role bucket -> the actual Power BI role key
    # expected by that visual family. This is the piece that was missing
    # before: every field was being captured, but under invented names like
    # "Legend"/"X" that don't match what Power BI actually expects for that
    # visual type (e.g. bar/line/area charts use "Series" not "Legend"; pie
    # and donut have no series role at all - color comes from Category;
    # tables/cards/slicers put every field under "Values"; matrix visuals
    # use "Rows"/"Columns"/"Values"; treemaps use "Group"/"Details"/"Values").
    _ROLE_PROFILES = {
        # Category (axis) + Series (legend/breakdown) + Y (measures)
        "bar_line": {"primary": "Category", "series": "Series", "y": "Y", "size": "Size", "tooltip": "Tooltips", "detail": "Details"},
        # Pie/donut charts have no separate "Category" role - Power BI's
        # actual field well for these is called "Legend", it accepts
        # multiple fields (both primary and any secondary dimension land
        # here), and measures go under "Values" (not "Y").
        "pie": {"primary": "Legend", "series": "Legend", "y": "Values", "size": "Tooltips", "tooltip": "Tooltips", "detail": "Details"},
        # Funnel/waterfall keep the conventional "Category"/"Y" naming.
        "funnel_waterfall": {"primary": "Category", "series": "Details", "y": "Y", "size": "Tooltips", "tooltip": "Tooltips", "detail": "Details"},
        "scatter": {"primary": "Details", "series": "Category", "y": "Y", "size": "Size", "tooltip": "Tooltips", "detail": "Details"},
        # Everything collapses onto a single "Values" role (table, card, slicer, gauge).
        "values_only": {"primary": "Values", "series": "Values", "y": "Values", "size": "Values", "tooltip": "Values", "detail": "Values"},
        "matrix": {"primary": "Rows", "series": "Columns", "y": "Values", "size": "Values", "tooltip": "Values", "detail": "Values"},
        "treemap": {"primary": "Group", "series": "Details", "y": "Values", "size": "Values", "tooltip": "Tooltips", "detail": "Details"},
        "map": {"primary": "Category", "series": "Series", "y": "Y", "size": "Size", "tooltip": "Tooltips", "detail": "Tooltips"},
        "decomp": {"primary": "Explain By", "series": "Explain By", "y": "Analyze", "size": "Analyze", "tooltip": "Analyze", "detail": "Explain By"},
    }
 
    # visualType -> which role profile family it belongs to.
    _FAMILY_BY_TYPE = {
        **{t: "bar_line" for t in (
            ...
            "lineChart", "areaChart", "stackedAreaChart", "ribbonChart",
            "lineClusteredColumnComboChart", "lineStackedColumnComboChart",
        )},
        **{t: "pie" for t in ("pieChart", "donutChart")},
        **{t: "funnel_waterfall" for t in ("funnel", "waterfallChart")},
        "scatterChart": "scatter",
        **{t: "values_only" for t in ("card", "multiRowCard", "kpi", "slicer", "tableEx", "gauge")},
        "pivotTable": "matrix",
        "treemap": "treemap",
        **{t: "map" for t in ("map", "filledMap", "shapeMap")},
        "Decomposition Tree": "decomp",
        "keyDriversVisual": "decomp",
    }
 
    # Role targets whose value should stay a single field object (matching
    # how "Category" already appears in existing outputs) rather than a
    # list, when only one field ends up there.
    _SINGULAR_TARGETS = {"Category", "Group"}
 
    @classmethod
    def _role_profile(cls, v_type: str) -> Dict[str, str]:
        family = cls._FAMILY_BY_TYPE.get(v_type, "bar_line")
        return cls._ROLE_PROFILES[family]
    ...
    def _build_bindings(self, dims: List[Dict[str, Any]], meas: List[Dict[str, Any]], v_type: str) -> Dict[str, Any]:
        """Dynamically builds bindings so that (a) no field/role is ever
        dropped, and (b) every field lands under the role KEY NAME that
        Power BI actually expects for this specific visual type - which is
        what determines whether the binding is valid, not just whether the
        field was captured at all.
        """
        profile = self._role_profile(v_type)
        is_matrix = v_type == "pivotTable"
        ...
        series_fields: List[Dict[str, Any]] = []
 
        if is_matrix and len(leftover_dims) >= 2:
            # Matrix-style visual: split remaining dims into Rows + Columns
            # instead of collapsing everything into a single field.
            primary_fields = leftover_dims[:-1]
            series_fields = [leftover_dims[-1]]
        else:
            if leftover_dims:
                primary_fields = [leftover_dims[0]]
                # Any dimension beyond the first is a second role (commonly
                # the Legend/series breakdown) - never dropped.
                series_fields = leftover_dims[1:]
 
        series_fields = series_fields + role_buckets["legend"] + role_buckets["color"]
        tooltip_dim_fields = list(role_buckets["tooltip"])
        detail_dim_fields = role_buckets["detail"] + role_buckets["details"]
        size_dim_fields = list(role_buckets["size"])
 
        # Measures: route explicitly-tagged size/tooltip measures to their
        # own bucket, everything else is a value/"Y" measure.
        y_entries, size_measure_entries, tooltip_measure_entries = [], [], []
        for m in meas:
            role = self._classify_role(m)
            entry = self._measure_binding(m)
            if role == "size":
                size_measure_entries.append(entry)
            elif role == "tooltip":
                tooltip_measure_entries.append(entry)
            else:
                y_entries.append(entry)
 
        bindings: Dict[str, Any] = {}
 
        def push(role_key: str, entries: List[Dict[str, Any]]):
            if not entries:
                return
            target = profile.get(role_key)
            if not target:
                return
            bindings.setdefault(target, [])
            bindings[target].extend(entries)
 
        push("primary", [self._field_binding(f) for f in primary_fields])
        push("series", [self._field_binding(f) for f in series_fields])
        push("tooltip", [self._field_binding(f) for f in tooltip_dim_fields])
        push("detail", [self._field_binding(f) for f in detail_dim_fields])
        push("size", [self._field_binding(f) for f in size_dim_fields])
        push("y", y_entries)
        push("size", size_measure_entries)
        push("tooltip", tooltip_measure_entries)
 
        # Roles that conventionally hold a single field (e.g. "Category")
        # come back out as a plain object instead of a one-item list, to
        # match the shape the rest of the pipeline (and existing reports)
        # already expects.
        for target in self._SINGULAR_TARGETS:
            if target in bindings and len(bindings[target]) == 1:
                bindings[target] = bindings[target][0]
    ...
    def build_visual(
        ...
        # 2. SortBy - dynamic fallback across every role that could anchor a
        # sort, in priority order. Covers whichever role names this visual's
        # family actually uses (Y/Values/Analyze for measures first, then
        # whatever the primary axis role is called for this family).
        sort_by: Dict[str, Any] = {}
        for role in ("Y", "Values", "Analyze", "Category", "Rows", "Group", "Explain By", "Series", "Columns"):
            target = bindings.get(role)
            if not target:
                continue
            target_field = target[0] if isinstance(target, list) else target
            sort_by = {
                "target": target_field,
                "direction": "Descending" if role in ("Y", "Values", "Analyze") else "Ascending",
            }
            break
