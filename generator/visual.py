...
try:
    ...
    try:
        from utils import map_visual_type
    except ImportError:
        # Fallback if map_visual_type is not directly found in path
        def map_visual_type(raw_type: str) -> str:
            raw = str(raw_type or "").lower()
            if "pie" in raw:
                return "pieChart"
            elif "card" in raw or "text table" in raw or "kpi" in raw:
                return "card"
            elif "horizontal" in raw:
                return "barChart"
            elif "clustered" in raw:
...
class VisualGenerator:
    ...
    @staticmethod
    def _strip_and_clean_subtype(subtype: str, fallback_type: str = "") -> str:
        """
        Cleans and strips subtype names: removes parenthesized qualifiers
        and normalizes variations.
        """
        if not subtype:
            return fallback_type.strip()

        # Remove parenthesized phrases, e.g. (Filtered), (SUM(...))
        cleaned = re.sub(r"\(.*?\)|\[.*?\]", "", str(subtype)).strip()
        cleaned_lower = cleaned.lower()

        if any(k in cleaned_lower for k in ["clustered", "combined"]):
            return "Clustered Bar Chart"
        elif any(k in cleaned_lower for k in ["horizontal", "ranked", "colored"]):
            return "Horizontal Bar Chart"
        elif "stacked" in cleaned_lower:
            return "Stacked Bar Chart"
        elif any(k in cleaned_lower for k in ["pie", "proportion", "donut"]):
            return "Pie Chart"
        elif any(k in cleaned_lower for k in ["single-value", "kpi", "card", "text table"]):
            return "Card"
        elif any(k in cleaned_lower for k in ["line", "trend"]):
            return "Line Chart"

        return cleaned if cleaned else fallback_type
    ...
    def _resolve_visual_type(
        ...
        """
        Dynamically extracts and resolves the visual type:
        - For Bar Charts: sets Visual Type to the stripped, cleaned Subtype.
        - Maps Cards/KPIs/Pie Charts accurately.
        """
        raw_type = (
            visual_meta.get("visualType")
            or visual_meta.get("mark")
        ...
        raw_subtype = (
            visual_meta.get("subtype")
            or visual_meta.get("Subtype / Mark Style")
            or visual_meta.get("markStyle")
            or ws.get("subtype")
            or ws.get("Subtype / Mark Style")
            or ws.get("markStyle")
            or ""
        ).strip()

        clean_subtype = self._strip_and_clean_subtype(raw_subtype, fallback_type=raw_type)

        # Bar chart requirement: keep visual type as same as stripped subtype
        if "bar" in raw_type.lower() or "bar" in clean_subtype.lower():
            resolved = clean_subtype if clean_subtype else "Horizontal Bar Chart"
        elif any(k in raw_type.lower() for k in ["card", "text table", "kpi"]) or "card" in clean_subtype.lower():
            resolved = "Card"
        elif "pie" in raw_type.lower() or "pie" in clean_subtype.lower():
            resolved = "Pie Chart"
        else:
            resolved = clean_subtype if clean_subtype else raw_type

        # Pass through map_visual_type to ensure target PBIR schema naming
        return map_visual_type(resolved)
    ...
    @staticmethod
    def _normalize_name(name: Any) -> str:
        """Normalizes a sheet/visual name for fuzzy matching."""
        if not name:
            return ""
        s = str(name).strip().lower()
        s = re.sub(r"\s*\(\d+\)\s*$", "", s)   # trailing " (2)"
    ...
    def _find_worksheet(self, sheet_name: str) -> Dict[str, Any]:
        """Dynamically resolves a worksheet tolerant of naming mismatches."""
        if not sheet_name:
            return {}
    ...
    @staticmethod
    def _classify_role(field: Dict[str, Any]) -> str:
        """Reads role/shelf tags off a field dynamically."""
        raw = (
            field.get("visualRole")
            or field.get("fieldRole")
            or field.get("shelf")
    ...
    def _build_bindings(self, dims: List[Dict[str, Any]], meas: List[Dict[str, Any]], v_type: str) -> Dict[str, Any]:
        """Dynamically constructs bindings without dropping any dimensional roles."""
        bindings: Dict[str, Any] = {}
    ...
    def build_visual(
        ...
        """Constructs a single runtime visual object."""
        visual_meta = visual_meta if isinstance(visual_meta, dict) else {}
        ws = self._find_worksheet(sheet_name)

        prebuilt_bindings = visual_meta.get("bindings") if isinstance(visual_meta.get("bindings"), dict) else None

        # Resolve visual type with stripped/aligned subtype rules
        v_type = self._resolve_visual_type(visual_meta, ws)
